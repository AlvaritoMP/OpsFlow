...
-- Solo Personal de unidad. No recorre ATS/Opalosis.
...
BEGIN
  IF to_regclass('public.resources') IS NOT NULL THEN
    ...
    WHERE r.type = 'Personal'
      AND regexp_replace(coalesce(r.dni, ''), '[^0-9]', '', 'g') = p_dni
  ...
  RETURN coalesce(result, jsonb_build_object('tipoDocumento', 'DNI', 'nroDocumento', p_dni));
EXCEPTION
  WHEN OTHERS THEN
    RETURN coalesce(p_stored, '{}'::jsonb) || jsonb_build_object('tipoDocumento', 'DNI', 'nroDocumento', p_dni);
END;
$$;

-- Replica solo a la ficha del colaborador en la unidad. No encola ni toca Opalosis.
...
GRANT EXECUTE ON FUNCTION public.save_public_complementary_ficha(text, text, jsonb) TO anon, authenticated, service_role, postgres;
